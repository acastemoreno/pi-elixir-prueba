defmodule ReportePi.Pi.Points do
  ...
  defp request_channel({{:ok, webid}, state}, path) do
    case state |> get_in([path, :channel_pid]) do
      nil ->
        {:ok, channel_pid} =
          ReportePi.Pi.HttpClient.DynamicWebsocket.start_child("streams/" <> webid <> "/channel?includeInitialValues=True", path)
      ...
      channel_pid when channel_pid |> is_pid() ->
        {{:ok, "La conexión a PI ya existe"}, state}
    end
  end
